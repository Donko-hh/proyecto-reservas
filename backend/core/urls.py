"""
URL configuration for core project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path, include
from django.http import JsonResponse
from rest_framework import routers
from authapp.views import UserViewSet, login_view
from events.views import EventViewSet
from reservations.views import ReservationViewSet
from reports.views import ocupacion_report, asistencia_report

def root_view(request):
    return JsonResponse({
        'name': 'Sistema de Eventos y Reservas',
        'status': 'ok',
        'endpoints': ['/admin/', '/api/usuarios/', '/api/login/', '/api/eventos/', '/api/eventos/activos/', '/api/reservas/', '/api/reportes/ocupacion/', '/api/reportes/asistencia/']
    })

router = routers.DefaultRouter()
router.register(r'usuarios', UserViewSet, basename='usuarios')
router.register(r'eventos', EventViewSet, basename='eventos')
router.register(r'reservas', ReservationViewSet, basename='reservas')

urlpatterns = [
    path('', root_view),
    path('admin/', admin.site.urls),
    path('api/', include(router.urls)),
    path('api/login/', login_view),
]

urlpatterns += [
    path('api/reportes/ocupacion/', ocupacion_report),
    path('api/reportes/asistencia/', asistencia_report),
]